...
MainWindow::MainWindow(int init_height, int init_width, const char* title) :m_window(nullptr), 
    deltaTime(0.0f), currTime(0.0f)
{
    // note that this is not thread-safe.
    if (!m_singleton) [[unlikely]]
    {
        IOExtension::LogError(std::source_location::current(), "Try to use two GLFW windows simutanously.");
        abort();
    }
    m_singleton = false;

    GLFWwindow* newWindow = glfwCreateWindow(init_height, init_width, title, nullptr, nullptr);
    if (newWindow == nullptr) [[unlikely]]
    { 
        IOExtension::LogError(std::source_location::current(), "Fail to create GLFW window.");
        return;
    }
    ...
    if(gladLoadGLLoader(reinterpret_cast<GLADloadproc>(glfwGetProcAddress)) == 0) [[unlikely]]
        IOExtension::LogError(std::source_location::current(), "Fail to initialize GLAD.");
    if(!ImGui_ImplOpenGL3_Init("#version 330")) [[unlikely]]
        IOExtension::LogError(std::source_location::current(), "Fail to initialize ImGui for OpenGL3.");
    if(!ImGui_ImplGlfw_InitForOpenGL(newWindow, true)) [[unlikely]]
        IOExtension::LogError(std::source_location::current(), "Fail to initialize ImGui for GLFW.");
