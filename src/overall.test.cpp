...
void SetMVP(float width, float height, float near, float far, 
	Core::BasicTriRenderModel& model, Core::Camera& camera, Core::Shader& shader)
{
...
int main()
{
	...
	mainWindow.Register(
		[&shader, &model, &frontCamera, &mainWindow, near, far]() {
			...
			SetMVP(static_cast<float>(width), static_cast<float>(height),
				near, far, model, frontCamera, shader);
			model.Draw(shader);
		});
	...
	//	frontCamera.Rotate(
	//		static_cast<float>(yOffset) * frontCamera.mouseSensitivity, { 1, 0, 0 });
	//});
